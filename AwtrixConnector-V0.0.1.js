const SkriptVersion = "0.0.1"; //vom 11.03.2021 / Link zu Git:  / Forum: 
...
Awtrix.Notify = { text: "", multiColorText: "", fallingText: "", force: false, name: "IobrokerText", scrollSpeed: 60, icon: 1, color: [255, 255, 255], moveIcon: true, repeatIcon: false, duration: null, repeat: 1, rainbow: false, progress: null, progressColor: [], progressBackground: [], soundfile: 1, yeelight: [], barchart: [], linechart: [] }
...
async function SendNotification() {
    ...
    let TargetString = "{";
    for (let x in Awtrix.Notify) { //Alle properties in Notify durchgehen
        log("x=" + x + " Awtrix.Notify[x]=" + Awtrix.Notify[x])
        if (Awtrix.Notify[x].val != "" && typeof Awtrix.Notify[x].val != "undefined") { //Wenn Datenpunkt nicht leer
            if (x == "text") {
                log("a")
                TargetString += x + ":" + Awtrix.Notify[x] + ","
            } else {
                log("b")
                TargetString += x + ":" + JSON.stringify(Awtrix.Notify[x].val) + ","
            }
        }
...
function CreateTrigger() {
    ...
    on({ id: Awtrix1Root + ".response", change: "any", ack: true }, function (dp) { //Trigger für Response erzeugen
        ...
        let DummyObj = JSON.parse(dp.state.val);
        ...
        //Basics
            ...
            if (logging) log("Error at answer #" + answerCounter, "error")
