const Version = "0.1.1" //vom 19.04.2021 - Skript um eine Awtrix als Melder zu verwenden
...
for (let x in Notifications.climate) {
    if (Notifications.climate[x].oid != "") {
        on(Notifications.climate[x].oid, function (dp) { //Trigger für Temperatur erzeugen
            ...
            if (x.indexOf("humidity") != -1) Notifications.climate[x].value = parseInt(dp.state.val);
        });
    };
}
